...
fun FloatingCircles(
    ...
    // In OLED pure black mode, keep background completely pitch black to prevent background glare patches and maximize battery savings
    if (isDark && isOled) return
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(Color(0xFF3B82F6).copy(alpha = 0.03f), Color.Transparent),
                center = Offset(x2, y2),
                radius = 140f
            ),
            center = Offset(x2, y2),
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(Color(0xFFF59E0B).copy(alpha = 0.03f), Color.Transparent),
                center = Offset(x3, y3),
                radius = 150f
            ),
            center = Offset(x3, y3),
